...
mod test {
    ...
    #[test]
    fn test_1() -> Result<(), Error>{    // 使用serde_json::to_string实现序列化
    ...
    #[test]
    fn test_2() -> Result<(), Error> {   // 使用serde_json::from_str实现反序列化
    ...
    #[test]
    fn test_3() -> Result<(), Error> {  // 序列化到文件
        let user = User::new("zhangsan", 12);
        let mut  file = fs::OpenOptions::new().write(true).create(true).open(PathBuf::from("E:\\project\\rust\\rust-study\\src\\serialize\\writer.json")).unwrap();
    ...
    #[test]
    fn test_4() -> Result<(), Error> { // 从文件反序列化成对象
        let file = fs::OpenOptions::new().read(true).open(PathBuf::from("E:\\project\\rust\\rust-study\\src\\serialize\\writer.json")).unwrap();
