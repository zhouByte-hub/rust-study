...
mod walkdir_test{
    ...
    fn test_2() {
        ...
        for item in WalkDir::new("E:\\project\\rust\\rust-study\\rust_async").into_iter().filter_map(|e| e.ok()) {
    ...
    fn test_3() {
        // 显示符号链接
        for item in  WalkDir::new("E:\\project\\rust\\rust-study\\rust_async").follow_links(true).into_iter().filter_map(|e| e.ok()){
            println!("{:?}-{}", item.path(), item.depth());
        }
    }
}