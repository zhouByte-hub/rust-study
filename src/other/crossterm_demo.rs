...
mod crossterm_test{
    ...
    use crossterm::style::{SetForegroundColor,SetBackgroundColor,Print, Color,ResetColor};   
    use crossterm::ExecutableCommand;
    
    #[test]
    fn test_1(){
        execute!(std::io::stdout(), SetBackgroundColor(Color::Red),Print("hello world")).unwrap();
        std::io::stdout()
            .execute(SetForegroundColor(Color::Blue)).unwrap()
            .execute(SetBackgroundColor(Color::Red)).unwrap()
            .execute(Print("Styled text here.")).unwrap()
