#[cfg(test)]
mod mysql_test{
    use sea_orm::{prelude::Expr, ActiveModelTrait, ActiveValue, ColumnTrait, Database, EntityTrait, QueryFilter, QueryOrder};
    use crate::database::models::dict;


    #[tokio::test]
    async fn connect_test(){
        let _db = Database::connect("mysql://root:123456@127.0.0.1:3306/driver_test").await.expect("数据库连接失败");
        println!("数据库连接成功");
    }

    #[tokio::test]
    async fn insert_test(){
        let db = Database::connect("mysql://root:123456@127.0.0.1:3306/driver_test").await.expect("数据库连接失败");
    ...
    async fn query_all_test() {
        let db = Database::connect("mysql://root:123456@127.0.0.1:3306/driver_test").await.expect("数据库连接失败");
        let list = dict::Entity::find().order_by_asc(dict::Column::Id).all(&db).await.unwrap();
        for item in list {
            println!("{:?}", item);
        }
    }
    ...
    async fn filter_query_test(){
        let db = Database::connect("mysql://root:123456@127.0.0.1:3306/driver_test").await.expect("数据库连接失败");
        let list = dict::Entity::find().filter(dict::Column::GroupCode.eq("CART_TYPE")).all(&db).await.unwrap();
        for item in list{
            println!("{:?}", item);
        }
    }
    ...
    async fn update_test_2(){
        ...
        let db = Database::connect("mysql://root:123456@127.0.0.1:3306/driver_test").await.expect("数据库连接失败");
        let update = dict::Entity::update_many().col_expr(dict::Column::DictCode, Expr::value("update_test_2")).filter(dict::Column::Id.eq(35));
    ...
    async fn filter_delete_test(){
        let db = Database::connect("mysql://root:123456@127.0.0.1:3306/driver_test").await.expect("数据库连接失败");
        let result = dict::Entity::delete_many().filter(dict::Column::Id.eq(34)).exec(&db).await.unwrap();
        println!("{:?}", result);   // DeleteResult { rows_affected: 1 }
    }
